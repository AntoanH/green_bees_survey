    ...
    <body>
        <div class="content" id="Main-Content">
            <form method="post">
                <div id="userinfo" style="width: 100%;
                     ...
                     <?php
                     if ($session_id != "")
                         echo "Welcome back " . $company_name . ",<br>This is your shareable link:   http://51.75.249.227/?s=" . $session_id;
                     else
                         echo "";
                     ?></div>
                <div id="q1" class="pricing tabcontent" ><div class="pricing-top blue-top">
                    ...
                    <div class="grid-w3layouts1" style="padding: 0 2em;background: #3c9841bd !important;
        ...
        <script>
            ...
            function nextQuestion() {
                if ($current_question > 0) {
                    ...
                    if (first) {
                        first = false;
                        if ($counter > 0)
                            setQuestion(getQuestionByNumber($answers[$counter + 1][0]));
                        else
            ...
            function prevQuestion() {
                if ($counter > 0) {
                    if ($answers.length > 0 && $answers.length > $counter) {
                        $next_question = getQuestionByNumber($answers[--$counter][0]);
                        console.log("Counter: " + $counter);
                        console.log("Question: " + $current_question);
                    } else {
                        if ($answers.length > 0) {
                            $next_question = getQuestionByNumber($answers[(--$counter) - $answers.length][0]);
                            console.log("Counter: " + $counter);
                            console.log("Question: " + $current_question);
                        } else {
                            $next_question = getQuestionByNumber($answers[--$counter][0]);
...
//                console.log($question);
            ...
            function deleteOldAnswer($question_number) {
                $answers = $.grep($answers, function (n, i) {
                    return n[0] != $question_number;
                });
            }

            function addAnswer($choice_id, $value) {
                deleteOldAnswer($current_question);
                if ($current_question != 0 && ($answers.length == 0 || ($answers.length > 0 && $current_question != $answers[$answers.length - 1][1]))) {
                    $answer = [];
                    $answer[0] = $current_question;
                    $answer[1] = $choice_id;
                    $answer[2] = $value;
                    $answers.push($answer);
                }
